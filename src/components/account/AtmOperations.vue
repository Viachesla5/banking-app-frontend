  ...
  methods: {
    ...
    async handleAtmOperation() {
      ...
      try {
        ...
        const operationText = this.operationType === 'deposit' ? 'deposited to' : 'withdrawn from';
        const formattedIban = this.formatIban(this.selectedAccount);
        this.success = `Transaction successful! €${formattedAmount} has been ${operationText} ${formattedIban}`;
          ...
          switch (error.response.status) {
            ...
            case 400:
              ...
              if (typeof error.response.data === 'object' && error.response.data.message) {
                ...
                this.error = "Invalid transaction. Please check your details.";
              }
              break;
            case 403:
